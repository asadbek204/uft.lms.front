...
function StudentsForm() {
  ...
  async function onFinish(data: any) {
    try {
      ...
      // Tug'ilgan sanani YYYY-MM-DD ga o'zgartirish
      if (data.student?.birthday) {
        const parts = data.student.birthday.split('.');
        if (parts.length === 3 && parts.every(p => p.length === 2 || p.length === 4)) {
          const [day, month, year] = parts;
          data.student.birthday = `${year}-${month}-${day}`;
        }
      }

      // Pasport berilgan sanani ham
    ...
    <div className="px-10 w-full mt-12 md:mt-0">
      ...
      >
        ...
        <div className="md:grid grid-cols-4 gap-x-[30px] gap-y-[10px] mb-6">
          ...
          <Form.Item label={contents.first_name} name="first_name" rules={[{ required: false, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.sure_name} name="surname" rules={[{ required: false, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.phone_number} name="phone_number" rules={[{ required: false, message: contents.required }]}>
            <PhoneInput />
          </Form.Item>
          <Form.Item
          ...
          >
            <Input placeholder="AA1234567" maxLength={9} onChange={(e) => form.setFieldValue("passport_seria", e.target.value.toUpperCase())} />
          </Form.Item>
          <Form.Item
          ...
          <Form.Item label={contents.pasportdate} name="passport_date" rules={[{ required: false, message: contents.required }]}>
            <DateInput />
          </Form.Item>
          <Form.Item label={contents.passport_address} name="passport_address" rules={[{ required: false, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item className="col-span-2" label={contents.address} name="address" rules={[{ required: false, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.price} name="price" rules={[{ required: true, message: contents.required }]}>
            <InputNumber min={0} style={{ width: "100%" }} />
          </Form.Item>
          <Form.Item label={contents.discount} name="discount" rules={[{ required: false, message: contents.required }]}>
            <InputNumber min={0} style={{ width: "100%" }} />
          </Form.Item>
        </div>
        ...
        <div className="md:grid grid-cols-3 gap-x-[30px] gap-y-[10px] mb-4">
          <Form.Item label={contents.last_name} name={["student", "last_name"]} rules={[{ required: true, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.first_name} name={["student", "first_name"]} rules={[{ required: true, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.sure_name} name={["student", "sure_name"]} rules={[{ required: true, message: contents.required }]}>
            <Input />
          </Form.Item>
          <Form.Item label={contents.phone_number} name={["student", "phone_number"]} rules={[{ required: true, message: contents.required }]}>
            <PhoneInput />
          </Form.Item>
