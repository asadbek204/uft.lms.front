...
const contentsMap = new Map<Langs, TLibraryComponentContent>([
  [Langs.UZ, { title: "Kitoblar", button: "Kitobni yuklash", categories: "Barcha kategoriyalar" }],
  [Langs.RU, { title: "Книги", button: "СКАЧАТЬ КНИГУ", categories: "Все категории" }],
  [Langs.EN, { title: "Books", button: "Download book", categories: "All categories" }],
]);
...
function LibraryPage() {
  ...
  const [originalBooks, setOriginalBooks] = useState<TBooks[]>([]); // Asl ma'lumotlar
  ...
  const handleUpdate = (newItem: any) => {
    ...
    setOriginalBooks((prev) => {
      ...
      const uniqueCategories = Array.from(new Set(updated.map((book) => book.category))).sort();
      setCategories(uniqueCategories);
      return updated;
    });
  };
  ...
  // Filtrlangan va saralangan kitoblar (useMemo orqali optimallashtirilgan)
  const displayedBooks = useMemo(() => {
    let filtered = originalBooks;

    // Kategoriya bo'yicha filtr
    if (sortCategory) {
      filtered = filtered.filter((book) => book.category === sortCategory);
    }

    // Kategoriya nomi bo'yicha alfabetik saralash
    return [...filtered].sort((a, b) => a.category.localeCompare(b.category));
  }, [originalBooks, sortCategory]);
    ...
    <div className="w-full mt-12 md:mt-0">
      ...
      <div className="2xl:h-[88%] h-[75%] overflow-y-auto">
        {displayedBooks.length === 0 ? (
          <p className="text-center text-gray-500 mt-10">Kitob topilmadi</p>
        ) : (
