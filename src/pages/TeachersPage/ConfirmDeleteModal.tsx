...
const contentsMap = new Map<Langs, TNewsComponentContent>([
    [Langs.UZ, {
        toast1: "O'qituvchi muvaffaqiyatli oʻchirildi",
        toast2: "O'qituvchini o‘chirib bo‘lmadi",
    ...
    [Langs.RU, {
        ...
        toast2: "Не удалось удалить преподавателя.",
    ...
    [Langs.EN, {
        toast1: "Teacher deleted successfully",
        toast2: "The teacher could not be deleted",
...
function ConfirmDeleteModal({ content, onDelete }: {content: number, onDelete: (content: number) => void}) {
    ...
    const deleteCourses = async (id: number) => {
        if (canDelete) {
            await client.delete(`employees/delete/${id}/`)
                .then(() => {
...
{open && (
  <div className="fixed inset-0 flex items-center justify-center z-50">
    <div className="absolute inset-0 bg-black bg-opacity-50" onClick={handleOpen}></div>
