        ...
        (async () => {
            ...
            try {
                ...
                const mappedVideos = fetchedVideos.map((video) => ({
                    id: video.id,
                    date: video.date,
                    file: video.video?.file || "https://www.shutterstock.com/shutterstock/videos/3573578167/preview/stock-footage-no-video-camera-recording-icon-sign-animated-on-a-black-background.mp4",
                    module: {
        ...
        <div className="w-full mt-12 md:mt-0">
            ...
            <div className="w-full 2xl:h-[88%] h-[75%] overflow-y-auto">
                ...
                {loading ? (
                    ...
                    {videos.length === 0 ? (
                      <p className="text-center text-xl text-gray-600 dark:text-gray-400">{contents.novedeos}</p>
                    ) : (
                      videos.map((video) => (
                        <div
                          key={video.id}
                        ...
                        >
                          ...
                          >
                            <div>
                              <h1 className="main-text text-xl sm:text-2xl lg:text-3xl 2xl:text-4xl dark:text-white">{video.unit}</h1>
                              <div className="flex items-center gap-2 mt-2">
                                <img className="w-4 h-4 filter dark:invert" src={calendarIcon} alt="Calendar Icon" />
                              ...
                              <p className="text-sm sm:text-lg dark:text-white">
                                {contents.description}: {video.video.description}
                              </p>
                            </div>
                          </div>
                        </div>
